...
TS_Current_Version=$(docker exec centos-ts3 cat /var/log/teamspeak/CurrentTSversion)
...
if [ $TS_New_Version = $TS_Current_Version ]
	then
		...
		TS3USER=$(docker exec $TS3CONTAINER grep User /var/log/teamspeak/Details | cut -d "=" -f 2)
		TS3PATH=$(docker exec $TS3CONTAINER grep Path /var/log/teamspeak/Details | cut -d "=" -f 2)
		echo $TS3USER;echo $TS3PATH
		tar xvf teamspeak3-server_linux_amd64-$TS_New_Version.tar.bz2
		docker stop $TS3CONTAINER
		docker cp ./teamspeak3-server_linux_amd64 $TS3CONTAINER:/opt/teamspeak3server/
		docker start $TS3CONTAINER
		sleep 1
		docker exec $TS3CONTAINER chown -R $TS3USER: $TS3PATH
		echo $TS_New_Version > CurrentTSversion
		docker cp CurrentTSversion $TS3CONTAINER:/var/log/teamspeak/CurrentTSversion
		docker exec $TS3CONTAINER kill -9 1
		exit 0
fi
exit 255
