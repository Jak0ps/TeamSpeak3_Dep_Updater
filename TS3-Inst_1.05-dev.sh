...
if [ $(head -n 1 /etc/issue | cut -d " " -f 1) = Ubuntu ] || [ $(head -n 1 /etc/redhat-release | cut -d " " -f 1) = CentOS ] || [ ! $(head -n 1 /etc/redhat-release | cut -d " " -f 1) = Red ]
then
echo "Welcome to teamspeak3 installer by Jacob Saleh :)"
else
echo "Please run this script only from an Ubuntu/Centos or RedHat distribution"
exit 400
fi
...
mv -r $TS3PATH/teamspeak3-server_linux_amd64 /tmp/
sudo systemctl stop ts3server
tar xvf teamspeak3-server_linux_amd64-$TS_New_Version.tar.bz2
cp -f /tmp/teamspeak3-server_linux_amd64/ts3server.sqlitedb* $TS3PATH/teamspeak3-server_linux_amd64/
...
if [ ! -d $TS3PATH ] && [ $(whoami) = root ]
    then
            mkdir -p $TS3PATH
			if command -v yum &> /dev/null ; then INSTCOMM=yum ; elif command -v apt &> /dev/null ; then INSTCOMM=apt ; fi
            if ! command -v curl &> /dev/null ; then $INSTCOMM install curl -y; fi
            if ! command -v wget &> /dev/null ; then $INSTCOMM install wget -y; fi
            if ! command -v tar &> /dev/null ; then $INSTCOMM install tar -y; fi
            if ! command -v bzip2 &> /dev/null ; then $INSTCOMM install bzip2 -y; fi
			...
			if [ $(head -n 1 /etc/issue | cut -d " " -f 1) = Ubuntu ]
			then
            adduser \
            --system \
            --shell /bin/bash \
